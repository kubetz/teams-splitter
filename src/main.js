...
const TEAMS = 16;    // number of teams
const STATIONS = 8; // number of stations
const ROUNDS = 8;   // number of rounds
...
// calculate for partially solved team 30
...
function calculate(start = 0, stations = _.times(STATIONS, _.stubArray), teams = _.times(TEAMS, _.stubArray)) {
  ...
  for(let team = start; team < TEAMS; team++) {
    ...
    result = assign(team, stations, teams, [])();
...
function assign(team, stations, teams, friends) {
  // we return function to save the parameters in a closure
  return function inner(station_start = 0, size = 2) {
    ...
    for (let station = station_start; station < STATIONS; station++) {
        ...
        let result = inner(station + 1) || inner(station + 1, 3);
